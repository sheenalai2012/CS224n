...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    ### TODO
    # query, key is size [bs, num_attention_heads, seq_len, attention_head_size]
    dk = torch.tensor(query.shape[-1])
    scores = torch.matmul(query, torch.transpose(key, -2, -1))
    scores = scores / torch.sqrt(dk) # size is [bs, num_attention_heads, seq_len, seq_len]

    scores += attention_mask
    scores = F.softmax(scores, dim=-1)
    scores = torch.matmul(scores, value) # [bs, num_attention_heads, seq_len, attention_head_size]

    scores = torch.transpose(scores, -3, -2) # bs, seq_len, num_attention_heads * attention_head_size
    scores = torch.flatten(scores, 2) # bs, seq_len, num_attention_heads * attention_head_size 
    return scores
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    return ln_layer(transformed_output + input)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    ### TODO
    # multihead attention
    multi_att_output = self.self_attention.forward(hidden_states, attention_mask)

    # multihead attention add norm
    multi_att_output = self.add_norm(hidden_states, multi_att_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    # feedfoward
    ff_output = self.interm_dense(multi_att_output)
    ff_output = self.interm_af(ff_output)

    # feedforward add norm
    ff_output = self.add_norm(multi_att_output, ff_output, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    rtn = inputs_embeds + pos_embeds + tk_type_embeds
    return self.embed_dropout(self.embed_layer_norm(rtn))
