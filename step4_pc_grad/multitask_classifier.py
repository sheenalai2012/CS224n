...
class MultitaskBERT(nn.Module):
    ...
    def eval_sentiment(self, embeddings):
        output = self.common_lin_proj(embeddings)
    ...
    def eval_paraphrase(self, embeddings):
        output = self.common_lin_proj(embeddings)
        intermediate = self.pd_dropout(output)
    ...
    def eval_similarity(self, embeddings):
        output = self.common_lin_proj(embeddings)
        intermediate = self.sts_dropout(output)
...
def train_multitask(args):
    ...
    args.batch_size = 8 # FOR THE MEM ISSUES!
    ...
    for epoch in range(15):
        ...
        for i in range(iters):
            ...
            optimizer.pc_backward()  # calculate the gradient can apply gradient modification
